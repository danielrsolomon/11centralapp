...
router.get('/', auth_1.requireAuth, (0, auth_1.requireRole)(['admin']), async (_req, res, next) => {
    try {
        ...
        // Get total courses
        const { count: totalCourses, error: coursesError } = await supabase_1.supabase
            .from('courses')
            .select('*', { count: 'exact', head: true });
        if (coursesError) {
            return (0, error_handler_1.throwApiError)(coursesError.message, 400, coursesError.code);
        }
        // Get total lessons
        const { count: totalLessons, error: lessonsError } = await supabase_1.supabase
            .from('lessons')
            .select('*', { count: 'exact', head: true });
        if (lessonsError) {
            return (0, error_handler_1.throwApiError)(lessonsError.message, 400, lessonsError.code);
        }
        // Get total modules
        const { count: totalModules, error: modulesError } = await supabase_1.supabase
            .from('modules')
            .select('*', { count: 'exact', head: true });
        if (modulesError) {
            return (0, error_handler_1.throwApiError)(modulesError.message, 400, modulesError.code);
        }
        // Schedule Statistics
        ...
        const { count: monthlyAppointments, error: appointmentsError } = await supabase_1.supabase
            .from('appointments')
            .select('*', { count: 'exact', head: true })
            .gte('date', firstDayOfMonth)
            .lte('date', lastDayOfMonth);
        if (appointmentsError) {
            return (0, error_handler_1.throwApiError)(appointmentsError.message, 400, appointmentsError.code);
        }
        // Chat Statistics
        // Get total chat rooms
        const { count: totalChatRooms, error: chatRoomsError } = await supabase_1.supabase
            .from('chat_rooms')
            .select('*', { count: 'exact', head: true });
        if (chatRoomsError) {
            return (0, error_handler_1.throwApiError)(chatRoomsError.message, 400, chatRoomsError.code);
        }
        // Get messages from this month
        const { count: monthlyMessages, error: messagesError } = await supabase_1.supabase
            .from('chat_messages')
            .select('*', { count: 'exact', head: true })
            .gte('created_at', firstDayOfMonth);
        if (messagesError) {
            return (0, error_handler_1.throwApiError)(messagesError.message, 400, messagesError.code);
        }
        // Gratuity Statistics
        // Get tips from this month
        const { count: monthlyTips, error: tipsError } = await supabase_1.supabase
            .from('tips')
            .select('*', { count: 'exact', head: true })
            .gte('created_at', firstDayOfMonth);
        if (tipsError) {
            return (0, error_handler_1.throwApiError)(tipsError.message, 400, tipsError.code);
        }
        // Get total tip amount for this month
        const { data: tipData, error: tipAmountError } = await supabase_1.supabase
            .from('tips')
            .select('amount')
            .gte('created_at', firstDayOfMonth);
        if (tipAmountError) {
            return (0, error_handler_1.throwApiError)(tipAmountError.message, 400, tipAmountError.code);
        }
 ...
 * @access Admin
 */
router.get('/activity', auth_1.requireAuth, (0, auth_1.requireRole)(['admin']), async (_req, res, next) => {
    try {
        ...
        // User signups by day
        const { data: userSignups, error: userError } = await supabase_1.supabase
            .from('users')
            .select('created_at')
        ...
        if (appointmentError) {
            return (0, error_handler_1.throwApiError)(appointmentError.message, 400, appointmentError.code);
        }
        // Chat messages by day
        const { data: messages, error: messageError } = await supabase_1.supabase
            .from('chat_messages')
            .select('created_at')
            .gte('created_at', startDateStr)
            .order('created_at', { ascending: true });
        if (messageError) {
            return (0, error_handler_1.throwApiError)(messageError.message, 400, messageError.code);
        }
        // Course progress entries by day
        const { data: progressEntries, error: progressError } = await supabase_1.supabase
            .from('course_progress')
            .select('updated_at')
            .gte('updated_at', startDateStr)
            .order('updated_at', { ascending: true });
        if (progressError) {
            return (0, error_handler_1.throwApiError)(progressError.message, 400, progressError.code);
        }
