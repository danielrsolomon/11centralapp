...
router.get('/', auth_1.requireAuth, (0, auth_1.requireRole)(['admin']), async (_req, res, next) => {
    try {
        ...
        const { data: thisMonthData, error: thisMonthError } = await supabase_1.supabase.rpc('get_dashboard_metrics', {
            start_date: firstDayOfMonth,
            end_date: today.toISOString()
        });
        if (thisMonthError) {
            return (0, error_handler_1.throwApiError)(thisMonthError.message, 400, thisMonthError.code);
        }
        // Get counts for last month (for comparison)
        const { data: lastMonthData, error: lastMonthError } = await supabase_1.supabase.rpc('get_dashboard_metrics', {
            start_date: firstDayOfLastMonth,
            end_date: lastDayOfLastMonth
        });
        if (lastMonthError) {
            return (0, error_handler_1.throwApiError)(lastMonthError.message, 400, lastMonthError.code);
        }
 ...
 * @access Admin
 */
router.get('/recent-activity', auth_1.requireAuth, (0, auth_1.requireRole)(['admin']), (0, validation_1.validateQuery)(zod_1.z.object({
    ...
    try {
        ...
        const { data: newUsers, error: usersError } = await supabase_1.supabase
            .from('users')
        ...
        const { data: courseProgress, error: progressError } = await supabase_1.supabase
        ...
        // Get recent tips
        const { data: tips, error: tipsError } = await supabase_1.supabase
            .from('tips')
            .select(`
          id,
          amount,
