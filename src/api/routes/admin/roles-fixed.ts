...
import { throwApiError, sendApiError } from '../../middleware/error-handler';
...
router.get('/',
  requireAuth,
  requireRole(['admin']),
  authAsyncHandler(async (_req: AuthenticatedRequest, res: Response, next: NextFunction) => {
...
router.get('/:roleId',
  ...
  authAsyncHandler(async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
